        ...
        <div v-if="origin && destination && !suggestion.submitted">
          ...
          <p>
            <label>
              ...
              <hour-selector v-model="timeHour" />
              :
              <minute-selector v-model="timeMinute" />
            </label>
...
import HourSelector from '../components/hour-selector.vue';
import MinuteSelector from '../components/minute-selector.vue';
...
export default {
  ...
  watch: {
    ...
    arrivalTime() {
      this.timeHour = (this.arrivalTime) ?
        moment(new Date(this.arrivalTime - 480 * 60000)).format('HH') : '08';
      this.timeMinute = (this.arrivalTime) ?
        moment(new Date(this.arrivalTime - 480 * 60000)).format('mm') : '00';
    }
  },
  computed: {
    ...mapState(['origin', 'destination', 'arrivalTime', 'selectedRoute', 'similarRequests',
  ...
  methods: {
    ...mapMutations(['setOrigin', 'setDestination', 'setArrivalTime']),
    ...
    suggest() {
      ...
      .then(() => {
        if (!this.timeHour || !this.timeMinute) {
          throw new Error('Please select a valid arrival time!')
        }
        var timeMs = [this.timeHour, this.timeMinute];
