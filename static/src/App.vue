<template>
  <div>
    <div v-if="profile">
      ...
      <div v-if="profile.email.endsWith('@data.gov.sg')">
        <div v-if="adminProfile">
          (Admin)
          <button class="btn btn-default" @click="adminLogOut">Log out</button>
        </div>
        <div v-else>
          <button class="btn btn-default" @click="adminLogIn">Log in</button>
        </div>
      </div>
    </div>
    <div v-else>
...
import * as adminAuth0 from './utils/adminLogin';
...
export default {
  ...
  computed: {
    ...
    'loadingRequests', 'token', 'profile', 'suggestions', 'adminProfile', 'adminIdToken'])
  },
  created() {
    this.$lock = auth0Lock;
    ...
    adminAuth0.auth0Lock.on('authenticated', (authResult) => {
      adminAuth0.auth0Lock.getProfile(authResult.idToken, (error, profile) => {
        if (error) {
          alert("Your email could not be verified");
          return;
        }

        this.$store.commit('setAdminProfile', {profile, idToken: authResult.idToken});
      });
    })
